...
proc_pagetable(struct proc *p)
{
  ...
  // PTE_R | PTE_U：这是映射的权限标志，表示该页是只读（PTE_R）且用户态可访问（PTE_U）。
  if (mappages(pagetable, USYSCALL, PGSIZE,
    (uint64)(p->usyscall), PTE_R | PTE_U) < 0) {
    // TRAMPOLINE是一个特殊的页面，用于从用户态到内核态的上下文切换。
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    // TRAPFRAME保存了进程的中断帧信息，用于处理系统调用或中断。
...
uint64 pgaccess(void* pg, int number, void* store) {
  ...
  for (int i = 0; i < number; i++) {
    ...
    // 如果 PTE 存在且访问标志（PTE_A）被设置，更新结果变量 ans，将对应的比特位置为 1
    if (pte != 0 && ((*pte) & PTE_A)) {
      ans |= 1 << i;
      // 清除 PTE_A 标志
      *pte ^= PTE_A; 
  ...
  // 使用 copyout 将结果 ans 复制到用户空间地址 store。
