...
void* cowpagealloc(pagetable_t pgt, uint64 va)
{
  ...
  // 如果物理页面的引用计数为 1（即该页未被共享）
  // 则直接将页表项的写标志 PTE_W 置位，清除 COW 标志 PTE_F，并返回旧的物理地址
  ...
  // 如果物理页面的引用计数大于 1（即该页被共享）
  // 调用 kalloc 分配一个新的物理页面 new_pa
  else
  {
    void* new_pa = kalloc();
    if (new_pa == 0)
